...
public class LoginbuddyProviderAuthorize extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            request.getRequestDispatcher("demoserverUsername.jsp?session="+session).forward(request, response);
        } catch (Exception e) {
            LOGGER.warning("The authorization request was invalid");
            e.printStackTrace();
            response.setStatus(404);
            response.setContentType("application/json; charset=UTF-8");
            response.getWriter().println("\"error\":\"invalid_Request\", \"error_description\": \"the authorization request was invalid due to missing or invalid parameters!\"");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        session = "null".equals(session) ? null : session; // for some reason we get "null"
        String action = request.getParameter("action");

        Map<String, Object> sessionValues = null;
        if (session != null && session.trim().length() > 0) {
            sessionValues = (Map<String, Object>) LoginbuddyCache.getInstance().remove(session);
            if (sessionValues == null) {
                ...
                String sessionState = (String)sessionValues.get(Constants.SESSION.getKey());
                if (!session.equals(sessionState)) {
        ...
        try {
            ...
            if (path.endsWith("/login")) {
                ...
                if (email != null && email.trim().length() > 0) {
                    ...
                    LoginbuddyCache.getInstance().put(session, sessionValues);
                    request.getRequestDispatcher("demoserverAuthenticate.jsp?session="+session).forward(request, response);
                ...
                if (password != null && password.trim().length() > 0) {
                    ...
                    LoginbuddyCache.getInstance().put(session, sessionValues);
                    request.getRequestDispatcher("demoserverConsent.jsp?session="+session).forward(request, response);
